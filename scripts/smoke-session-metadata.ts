...
assert.equal(colorFromSerial('Neurex Red'), 'RED');
...
assert.equal(m.device_color, 'RED');
...
assert.equal(m.channel_count, 1);
assert.equal(m.fp1_index, 4);
...
// variantKnown=null (pre-v2 firmware) -> null, not false
const mUnknown = buildSessionMetadata({ deviceId: 'x', serial: 'Neurex Yellow', scale: { ...scale, variantKnown: null } });
assert.equal(mUnknown.variant_known, null);

// No scale (older firmware) -> still emits device + app version, no scale fields
