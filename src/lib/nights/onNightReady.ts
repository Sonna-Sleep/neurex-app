...
//   2. fire the local "Your night is ready" banner — but only when the app
//      isn't already in the foreground (no point banner-ing a user who's
//      looking at the result appear on screen)
//
...
export function handleNightReady(session: Session): void {
  ...
  if (AppState.currentState !== 'active' && !notified.has(session.id)) {
