...
describe('smart-wake timing', () => {
  const at = (h: number, m: number) => new Date(2026, 6, 2, h, m, 0, 0).getTime();

  it('arms the board for 15 minutes after the alarm, not the pre-alarm window', () => {
    expect(secondsUntilSmartWakeFallback(at(22, 0), { hour: 7, minute: 30 })).toBe(
      9.75 * 3600,
    );
  });

  it('derives all markers from one alarm occurrence', () => {
    const times = smartWakeTimes(at(22, 0), { hour: 7, minute: 30 });
    expect(times.alarmAtMs).toBe(at(7, 30) + 24 * 60 * 60 * 1000);
    expect(times.alarmAtMs - times.wakeWindowStartMs).toBe(30 * 60 * 1000);
    expect(times.fallbackDeadlineMs - times.alarmAtMs).toBe(15 * 60 * 1000);
  });
});
