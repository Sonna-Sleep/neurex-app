...
const mockSessionState = {
  ...
  smartWakeRuntime: null as any,
  smartWakeHistory: [] as any[],
  ...
  setSmartWakeRuntime: jest.fn((runtime) => {
    mockSessionState.smartWakeRuntime = runtime;
  }),
  patchSmartWakeRuntime: jest.fn((patch) => {
    if (!mockSessionState.smartWakeRuntime) return;
    mockSessionState.smartWakeRuntime = { ...mockSessionState.smartWakeRuntime, ...patch };
  }),
  appendSmartWakeHistory: jest.fn((point) => {
    mockSessionState.smartWakeHistory.push(point);
  }),
};
...
describe('streamController wake-light reconnect sync', () => {
  beforeEach(() => {
    ...
    mockSessionState.smartWakeRuntime = null;
    mockSessionState.smartWakeHistory = [];
  ...
  it('arms the device with the reviewed SET_ALARM_RELATIVE payload on session start', async () => {
    ...
    try {
      ...
      expect(device.writeAlarmControl).toHaveBeenCalledWith(
        encodeSetAlarm(9.75 * 3600, SUNRISE_RAMP_S, SUNRISE_MAX_BRIGHTNESS, SUNRISE_COLOR),
      );
    } finally {
      await stopSession();
