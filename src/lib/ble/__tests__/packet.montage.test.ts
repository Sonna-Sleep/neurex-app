...
describe('parsePacket compact montage decode', () => {
  test('decodes Fp1/Fp2/EOG-L/EOG-R channels and keeps fp1_uV', () => {
    const codes = {
      0: 100_000,
      1: 200_000,
      2: -50_000,
    ...
    expect(out.packet.samples.length).toBe(SAMPLES_PER_PACKET);

    for (const sample of out.packet.samples) {
      expect(Object.keys(sample.channels).sort()).toEqual([
        'EOG-L',
        'EOG-R',
        'Fp1',
        'Fp2',
  ...
  test('derives sample count from notification length', () => {
    ...
    expect(out.ok).toBe(true);
    if (!out.ok) throw new Error('unreachable');
    expect(out.packet.samples.length).toBe(3);
    expect(out.packet.samples[2].fp1_uV).toBe(102);

    const bad = makeSizedPacket(3);
    const checksumIdx = bad.length - PKT_TRAILER_BYTES;
    const shortened = bad.slice(0, checksumIdx - 1);
    expect(parsePacket(shortened, 0, 1, activeChannels(scale), STREAM_CHANNELS)).toEqual({
      ok: false,
      reason: 'size',
    });
  });
